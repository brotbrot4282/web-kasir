        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/20 backdrop-blur-sm" onClick={resetForm}>
          <form onSubmit={simpan} className="bg-white rounded-xl p-6 shadow-xl border border-sage-200 w-full max-w-md mx-4 space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div>
              ...
              <input type="text" value={form.namaBahan} onChange={(e) => setForm({ ...form, namaBahan: e.target.value })} className="w-full border border-sage-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sage-600/20 focus:border-sage-400 transition-all" placeholder="Misal: Mie Indomie" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-sage-600 mb-1">Jumlah</label>
                <input type="text" value={form.jumlah} onChange={(e) => setForm({ ...form, jumlah: e.target.value.replace(/\D/g, "") })} className="w-full border border-sage-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sage-600/20 focus:border-sage-400 transition-all" placeholder="0" />
              </div>
              <div>
              ...
              {stokList.map((item) => (
                <tr key={item.id} className="hover:bg-sage-50 transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${item.jumlah <= 5 ? "bg-red-50 text-red-500" : "bg-sage-100 text-sage-600"}`}>{item.jumlah}</span>
                  </td>
                  <td className="px-4 py-3.5 text-center text-sage-500">{item.satuan}</td>
                  <td className="px-4 py-3.5 text-center">
                    <button onClick={() => editStok(item)} className="text-sage-500 hover:text-sage-700 text-sm font-medium mr-2 transition-colors">Edit</button>
                    <button onClick={() => hapus(item.id)} className="text-sage-500 hover:text-red-500 text-sm font-medium transition-colors">Hapus</button>
                  </td>
                </tr>
              ))}
              {stokList.length === 0 && (
